    ...
    resolveId(importee, importer, resolveOptions){
        ...
        return this.resolve(updatedId, importer, Object.assign({ skipSelf: true}, resolveOptions)).then(resolved => {
            ...
            if(!isAbsolute(updatedId)){
                this.warn(`rewrote ${importee} to ${updatedId} but was not an absolute path and was not handled by other plugins. ` + `This will lead to duplicated modules for the same path. ` + `To avoid duplicating modules, you should resolve to an absolute path.`)
            }
            return {id: updatedId}
        })
    }
...
function getEntries({
  ...
  const resolveFunctionFromOptions = resolveCustomResolver(customResolver);
